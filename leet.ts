...
export async function getDailyProblem(): Promise<Result<ProblemList, Error>> {
    //offset
    const offsetResult = await resultify<Deno.KvEntryMaybe<number>>(
        kv.get<number>(OFFSET_KEY),
    );
    ...
    //difficulty
    const difficultyResult = await resultify<Deno.KvEntryMaybe<Difficulty>>(
    ...
    //store problem
    const storeResult = await resultify(
