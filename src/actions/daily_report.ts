...
const AcceptedSubmission = z.object({
  id: z.string(),
  titleSlug: z.string(),
  timestamp: z.string().transform((d) =>
    DateTime.fromSeconds(Number(d)).setZone("UTC")
  ),
});
...
async function getAcceptedByUsername(username: string) {
  ...
  }).then((d) => d.data.recentAcSubmissionList).then((subs) =>
    AcceptedSubmission.array().parse(subs)
  );
}
...
async function getUserSubmissionsForDate(username: string, unsafe_date: Date) {
  ...
  const result = await lc.graphql({
    ...
    SubmissionsCalendar.parse(
      JSON.parse(d.data.matchedUser.userCalendar.submissionCalendar),
    )
  );
...
export default async function daily_report(
  raw_day: Date,
  ...
  const put_result = await resultify(
  ...
  const message = `Daily report for ${day.toFormat("yyyy\\-MM\\-dd")}:\n\n${
    results.map((r) =>
      r.status === "fulfilled"
        ? `${
          r.value.username.replaceAll("-", "\\-").replaceAll("_", "\\_")
        }: ${r.value.subs}`
        : `${r.reason}`
    ).join("\n\\-\\-\\-\n")
  }`;
